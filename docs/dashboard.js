...
async function startApplication() {
  ...
  const env_spec = ['https://cdn.holoviz.org/panel/0.14.2/dist/wheels/bokeh-2.4.3-py3-none-any.whl', 'https://cdn.holoviz.org/panel/0.14.2/dist/wheels/panel-0.14.2-py3-none-any.whl', 'pyodide-http==0.1.0', 'holoviews>=1.15.1', 'matplotlib', 'numpy', 'pandas', 'plotly', 'seaborn']
...
# Creating bar plots using Plotly
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=True, gridcolor='lightgray'),  # Show grid lines for x-axis
        yaxis=dict(showgrid=True, gridcolor='lightgray')  # Show grid lines for y-axis
    )    
...
def osalejad(liik, multi_choice_value, slider_value):
    ...
    fig1 = go.Figure()
    fig1.add_trace(go.Bar(y=df_count['index'], x=df_count[1.0], orientation='h'))
    fig1.update_layout(
        title='Õnnetustes osalejate arv',
        xaxis_title='Kokku',
        yaxis_title='',
        xaxis=dict(showgrid=True, gridcolor='lightgrey'),  # Show x-axis gridlines with grey color
        yaxis=dict(showgrid=True, gridcolor='lightgrey'),  # Show y-axis gridlines with grey color
        plot_bgcolor='white'
    )
    ...
    if liik == "Osalejad":
        ...
        fig = go.Figure(go.Bar(y=õnnetusi.index, x=õnnetusi.values, orientation='h'))
        fig.update_layout(
            title='Erinevat liiki õnnetuste arv',
            xaxis_title='Õnnetuste arv',
            yaxis_title='',
            xaxis=dict(showgrid=True, gridcolor='lightgrey'),  # Show x-axis gridlines with grey color
            yaxis=dict(showgrid=True, gridcolor='lightgrey'),  # Show y-axis gridlines with grey color
            plot_bgcolor='white'
...
def perioodid_joonis(perioodid, slider_value, multi_choice_value):
    ...
    # Creating line plots using Plotly
    if perioodid == "Aastad":
        fig = go.Figure(data=go.Scatter(x=Aastad.index, y=Aastad.values, mode='lines+markers'))
    elif perioodid == "Kuud":
        fig = go.Figure(data=go.Scatter(x=Kuud.index, y=Kuud.values, mode='lines+markers'))
    elif perioodid == "Päevad":
        fig = go.Figure(data=go.Scatter(x=Päevad.index, y=Päevad.values, mode='lines+markers'))
    elif perioodid == "Tunnid":
        fig = go.Figure(data=go.Scatter(x=Tunnid.index, y=Tunnid.values, mode='lines+markers'))

    fig.update_layout(
        title='Õnnetusi perioodide kaupa',
        xaxis_title='Periood',
        yaxis_title='Õnnetuste arv',
        plot_bgcolor='rgba(0,0,0,0)',  # Set background color to transparent
        xaxis=dict(showgrid=True, gridcolor='lightgray'),  # Show grid lines for x-axis
        yaxis=dict(showgrid=True, gridcolor='lightgray')  # Show grid lines for y-axis
    )
...
# Create the CSS style for the dashboard background color
dashboard_color = {'background-color': '#C6DDE1'}
    ...
    pn.Row(
        pn.Column(
            ...
            pn.Row(
                ...
                pn.bind(statistika, slider, multi_choice, 'Hukkunuid'),
                align=('center')
            ),
            multi_choice,
            pn.Row(perioodid),
